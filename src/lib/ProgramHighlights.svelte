<script>
  import { programmeHighlights } from './queens.js';
</script>

<section id="highlights" class="highlights">
  <div class="container">
    <header>
      <p class="eyebrow">Programme Highlights · 14 – 31 August 2026</p>
      <h2>Seventeen days, <span class="italic">day by day.</span></h2>
      <p class="sub">
        The ceremonial itinerary as set out in the official invitation — from the
        Royal Arrival to the cultural finale at the Notting Hill Carnival.
      </p>
    </header>
...
<style>
  .highlights { background: var(--paper-soft, #f3ecdd); }
  ...
  .item {
    display: grid; grid-template-columns: 2.2rem 8.5rem 1fr; align-items: center;
    gap: 0.5rem 1rem; padding: 0.85rem 0.5rem; border-bottom: 1px solid var(--line, rgba(60,36,21,0.14));
  }
  .item:last-child { border-bottom: none; }
  .ico { font-size: 1.2rem; line-height: 1; justify-self: center; }
  .when {
    justify-self: start; box-sizing: border-box; width: 7.5rem;
  ...
  @media (max-width: 560px) {
    .item { grid-template-columns: 1.9rem 1fr; gap: 0.4rem 0.7rem; }
